...
func (r *Repository) CreateEvent(ctx context.Context, e *models.Event) error {
	query := `INSERT INTO events (user_id, event_date, title)
			  VALUES ($1, $2, $3)
			  RETURNING id, created_at, updated_at`
	return r.Pool.QueryRow(ctx, query, e.UserID, e.Date, e.Title).
		Scan(&e.ID, &e.CreatedAt, &e.UpdatedAt)
}

func (r *Repository) UpdateEvent(ctx context.Context, e *models.Event) error {
	query := `UPDATE events SET event_date=$1, title=$2 WHERE id=$3 RETURNING created_at, updated_at`
...
func (r *Repository) GetEventsForDay(ctx context.Context, userID int64, date time.Time) ([]models.Event, error) {
	query := `SELECT id, user_id, event_date, title, created_at, updated_at
...
func (r *Repository) GetEventsForRange(ctx context.Context, userID int64, from, to time.Time) ([]models.Event, error) {
	query := `SELECT id, user_id, event_date, title, created_at, updated_at
			  FROM events WHERE user_id=$1 AND event_date BETWEEN $2 AND $3 ORDER BY event_date`
...
func (r *Repository) fetch(ctx context.Context, query string, args ...interface{}) ([]models.Event, error) {
	...
	for rows.Next() {
		var e models.Event
		if err := rows.Scan(&e.ID, &e.UserID, &e.Date, &e.Title, &e.CreatedAt, &e.UpdatedAt); err != nil {
			return nil, err
		}
