...
func TestCreateEvent(t *testing.T) {
	...
	mock.ExpectQuery(`INSERT INTO events`).
		WithArgs(e.UserID, e.Date, e.Title).
...
func TestGetEventsForDay(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT id, user_id, event_date, title, created_at, updated_at`).
		WithArgs(int64(1), date).
		WillReturnRows(pgxmock.NewRows([]string{
			"id", "user_id", "event_date", "title", "created_at", "updated_at",
		}).AddRow(1, 1, date, "Meeting", time.Now(), time.Now()))
